...
/*
TODO: написать функцию для считывания, доделать эту хрень
...
fn main() {
    loop { //инициализация петли
        ...
        let a: [f64; 6] = [read_var(), read_var(), read_var(), read_var(), read_var(), read_var()];

        let a: f64 = calc(a); //переменная Y приравнивается к функции calc от X
        println!("{a}");
